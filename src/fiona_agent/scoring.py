...
def score_post(text: str, *, seen_hashes: set[str] | None = None) -> ScoreBreakdown:
    """
    ...
    # novelty: if we've seen the exact content hash, treat as not novel
    novelty = 0.7
    if seen_hashes is not None:
        h = str(hash(text.strip().lower()))
        novelty = 0.2 if h in seen_hashes else 0.7

    return ScoreBreakdown(relevance=relevance, novelty=novelty, quality=quality)
